...
export const AdminSpaceKitchen: FC<Props> = ({ space }) => {
    ...
    return (
        <>
            <h3 className='font-roboto text-xl text-vivvi font-semibold mb-4'> Cocina </h3>
            {
                            ...
                            <div className='flex gap-6 mt-4'>
                                ...
                                <div className='w-8/12 flex flex-col gap-2'>
                                    <label className=" p-2 bg-white border border-platinum flex items-center" >
                                        <select name='tipology' id={tipology.toString()} onChange={handleKitchen} defaultValue={newProject.spaces.filter(spa => spa.name === space.name)[0]?.tipologies.filter((typo) => typo.id === tipology)[0]?.tipology}>
                                            <option>
                                                Cocina en Barra
                                            </option>
                                            <option>
                                    ...
                                    <label className=" p-2 bg-white border border-platinum">
                                        <input type="text" name={"area"} id={tipology.toString()} className="w-full" placeholder='Área (m2)' defaultValue={newProject.spaces.filter(spa => spa.name === space.name)[0]?.tipologies.filter((typo) => typo.id === tipology)[0]?.area} onChange={handleKitchen} />
                                    </label>
                                    <label className=" p-2 bg-white border border-platinum">
                                        <input type="text" name={"lowCabinet"} id={tipology.toString()} className="w-full" placeholder='ML Mueble bajo' defaultValue={newProject.spaces.filter(spa => spa.name === space.name)[0]?.tipologies.filter((typo) => typo.id === tipology)[0]?.lowCabinet} onChange={handleKitchen} />
                                    </label>
                                    <label className=" p-2 bg-white border border-platinum">
                                        <input type="text" name={"highCabinet"} id={tipology.toString()} className="w-full" placeholder='ML Mueble alto' defaultValue={newProject.spaces.filter(spa => spa.name === space.name)[0]?.tipologies.filter((typo) => typo.id === tipology)[0]?.highCabinet} onChange={handleKitchen} />
                                    </label>
