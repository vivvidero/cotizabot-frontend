...
export const AdminSpaceDiningRoom: FC<Props> = ({ space }) => {
    ...
    const handleDiningRoom = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    return (
        <>
            <h3 className='font-roboto text-xl text-vivvi font-semibold'> Sala / Comedor {space.roomNumber === 1 ? '' : space.roomNumber} </h3>
                ...
                return (
                    <div key={tipology} className='flex flex-col gap-2'>
                        ...
                        <label className=" p-4 bg-white border border-platinum">
                            <input type="text" name={"area"} id={tipology.toString()} className="w-full" placeholder='Área (m2)' defaultValue={newProject.spaces.filter(spa => spa.name === space.name && spa.roomNumber === space.roomNumber)[0]?.tipologies.filter((typo) => typo.id === tipology)[0]?.area} onChange={handleDiningRoom} />
                        </label>
